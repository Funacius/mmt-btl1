...
class PeerClient:
    ...
    def __init__(self, peer_ip, peer_port, tracker_ip, tracker_port):
        ...
        :param peer_id (str): peer_id of this peer
        ...
        print("peer_id: {}".format(self.peer_id))
        ...
        self.peer_info = {}  # {peer_id: {"ip": str, "port": int}}
    ...
    def _handle_peer_connection(self, conn, addr):
        ...
        try:
            ...
            if msg_type == "handshake":
                # Peer identification
                peer_peer_id = message.get("peer_id", "unknown")
                
                print("[Peer] Handshake from peer: {}".format(peer_peer_id))
                
                # Store connection
                with self.connections_lock:
                    self.peer_connections[peer_peer_id] = conn
                ...
                self._listen_to_peer(conn, peer_peer_id)
    ...
    def _listen_to_peer(self, conn, peer_peer_id):
        """
        Continuously listen for messages from a connected peer.
        
        :param conn (socket): Connection socket
        :param peer_peer_id (str): peer_id of the peer
        """
        try:
            while self.running:
                conn.settimeout(1.0)
                try:
                    ...
                    print("[Peer] Error receiving from {}: {}".format(peer_peer_id, e))
                    break
        
        finally:
            # Remove connection
            with self.connections_lock:
                if peer_peer_id in self.peer_connections:
                    del self.peer_connections[peer_peer_id]
            ...
            print("[Peer] Disconnected from peer: {}".format(peer_peer_id))
    ...
    def connect_peer(self, peer_peer_id, peer_ip, peer_port):
        """
        Establish P2P connection to another peer.
        
        :param peer_peer_id (str): peer_id of target peer
        ...
        print("[Peer] Connecting to peer {} at {}:{}".format(
            peer_peer_id, peer_ip, peer_port))
        ...
        try:
            ...
            if response_data.get("type") == "handshake_ack":
                # Store connection
                with self.connections_lock:
                    self.peer_connections[peer_peer_id] = sock
                    self.peer_info[peer_peer_id] = {
                        "ip": peer_ip,
                        "port": peer_port
                    }
                
                print("[Peer] Connected to peer: {}".format(peer_peer_id))
                
                # Start listening thread for this peer
                listen_thread = threading.Thread(
                    target=self._listen_to_peer,
                    args=(sock, peer_peer_id)
                )
            ...
            print("[Peer] Failed to connect to {}: {}".format(peer_peer_id, e))
            return False
    
    
    def send_peer(self, peer_peer_id, message, channel="direct"):
        """
        Send message to a specific peer via P2P.
        
        :param peer_peer_id (str): Target peer peer_id
        :param message (str): Message content
        :param channel (str): Channel name (optional)
        :return: bool - True if sent successfully
        """
        with self.connections_lock:
            if peer_peer_id not in self.peer_connections:
                print("[Peer] Not connected to peer: {}".format(peer_peer_id))
                return False
            
            conn = self.peer_connections[peer_peer_id]
        ...
        try:
            ...
            print("[Peer] Sent message to {}: {}".format(peer_peer_id, message))
            return True
        
        except Exception as e:
            print("[Peer] Failed to send to {}: {}".format(peer_peer_id, e))
            return False
    ...
    def broadcast_peer(self, message, channel="broadcast"):
        ...
        for peer_peer_id, conn in peer_list:
            try:
                msg_data = {
                    "type": "broadcast",
                    "from": self.peer_id,
                ...
                print("[Peer] Failed to broadcast to {}: {}".format(peer_peer_id, e))
        ...
        try:
            ...
            if '\r\n\r\n' in response:
                ...
                if data.get("status") == "success":
                    ...
                    connected_count = 0
                    for peer in peers:
                        peer_peer_id = peer.get("peer_id")
                        peer_ip = peer.get("ip")
                        peer_port = peer.get("port")
                        
                        # Don't connect to yourself
                        if peer_peer_id == self.peer_id:
                            continue
                        
                        # Skip if already connected
                        with self.connections_lock:
                            if peer_peer_id in self.peer_connections:
                                print("[Peer] Already connected to: {}".format(peer_peer_id))
                                continue
                        
                        # Try to connect
                        if self.connect_peer(peer_peer_id, peer_ip, peer_port):
                            connected_count += 1
    ...
    def handle_quit(self):
        """
        Gracefully leave the network by notifying the tracker.
        Uses the same raw HTTP style as other tracker methods.
        """
        print("\n[Peer] Leaving the network...")

        try:
            # Gửi POST /leave tới tracker (nếu tracker hỗ trợ)
            body = json.dumps({
                "peer_id": self.peer_id
            })
            ...
            # Nhận phản hồi (không bắt buộc phải đọc hết, chỉ cần gửi là được)
            sock.recv(4096)
            sock.close()

            print("[Peer] Notified tracker - removed from peer list")

        except Exception as e:
            # Nếu tracker không có /leave hoặc lỗi → vẫn thoát bình thường
            print("[Peer] Could not notify tracker (continuing shutdown): {}".format(e))

        # Đóng tất cả kết nối P2P
        with self.connections_lock:
            for peer_id, conn in list(self.peer_connections.items()):
                try:
        ...
        self.running = False
        print("[Peer] Goodbye! You have left the network cleanly.")
    ...
    try:
        while True:
            ...
            if command == "quit":
                ...
                ip = parts[2]
                port = int(parts[3])
                peer.connect_peer(peer_id, ip, port)
            
            elif command == "send" and len(parts) >= 3:
                peer_id = parts[1]
                message = " ".join(parts[2:])
                peer.send_peer(peer_id, message)
