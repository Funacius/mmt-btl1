...
from start_app import PeerClient
...
def get_channel_list(headers="guest", body="anonymous"):
    ...
    """
    print("[WebPeer] Client requested channel list")
    
    # Danh sách channel cố định (có thể thay bằng lấy từ tracker sau)
    channels = ["general", "random", "tech-talk", "homework", "offtopic", "bkchat"]
...
@app.route('/getlist', methods=['OPTIONS'])
def getlist_options(headers="guest", body="anonymous"):
    return ("200 OK", {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, OPTIONS",
